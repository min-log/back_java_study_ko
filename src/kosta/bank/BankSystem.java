	...
	public void showMenu(){		//show method 만들기
		...
		do{
			...
			if(menu.equals("1")){				//고객등록
				...
				// 혹시 account 클래스로 바로 접근하는 호출이 있을까..?
				// 있다면, addCustomer 이 안에 넣는 것은 매우 비효율적일텐데!
				// 없다면, account 클래스로 유일하게 이어지는 방법이구나!
			}
				...
				if(cust == null){
					...
					// cust.getAccount() == account 똑같다!
					// 생성자는 클래스가 호출되면 같이 실행된다는 사실! 잊지않기!!!
				...
				if(cust == null) {
					...
					System.out.println("잔고는 " + cust.getAccount().getBalance());
				}
			}
